...
Besides the rvalue reference to rvalue reference, the rest of three are all result to a lvalue reference. Thus, when invoking `func(str)`, `T` is `std::string&` and `T&&` results `std::string&`, so argument `v` is a lvalue reference.
