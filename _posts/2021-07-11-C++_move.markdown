...
```
A rvalue reference is in form of `T&&` (lvalue reference is `T&`). A rvalue reference can only bind to a rvalue but unable to bind to a lvalue. Consider the case below.
...
```
The invoking such as `foo("1")` and `foo(str + "1")` is valid. but `foo(str)` isn't allowed, because it tries to bind a rvalue reference to `str`, which is a lvalue. Now we have problem. Assume the string `str` will no longer be used after invokes `foo(str)`. apparently `str` can be treated as "dying", we don't want have an extra copy, so we hope it can be cast to a rvalue. So that's why we need `std::move`.
...
func(str + "4"); // OK
```
In order to understand these codes, we should first learn about template deduction. When we pass `str` to `func`, complier notices that it is a lvalue, so it deduces the template parameter `T` as a lvalue reference type. The argument `T&& v` then seems become a rvalue reference to a lvalue reference of `std::string`. Wait, we all know there is no a reference to a reference, but here are special rules called "Reference Collapsing". If we use template or type alias to try to create "a reference to a reference", then it will be collapsed to a ordinary lvalue or rvalue reference.

There are 4 rules of reference collapsing, which listed below. We denote the `TR` is the reference type to `T`, meanwhile the `T` is the reference type to `A`. The collapsing will result a reference type to `A`.
...
Besides the rvalue reference to rvalue reference, the rest of three will are result to a lvalue reference. Thus, when invoking `func(str)`, `T` is `std::string&` and `T&&` results `std::string&`, so argument `v` is a lvalue reference.
...
Now let's move on to the `std::move`, and see a simple case first. We implemented a func `MyMove`, but it's a bad implementation compares to the `std::move`. Recall the the effect of `std::move` is casting to rvalue.
...
```
The argument `__t` has type of `_Tp&&`, which `_Tp` is template parameter. This is a universal reference so it can accepts both lvalue and rvalue. We don't need to know the details of `std::remove_reference` at present. Just remember if there is a reference type, then it results a dereference type. So `typename std::remove_reference<_Tp>::type&&` is a rvalue reference.
...
There is a truth that might be counterintuitive is that **a rvalue reference itself is a lvalue**. It's confusing for beginners, let's just see a case below.
...
`r` is a rvalue reference. If we want to invoke the overload of `Call` whose argument is rvalue reference, just use `std::move`. However, consider a situation of universal references, we can't figure out whether the argument is lvalue reference or rvalue reference. For example.
...
int v = 123;
CallTwo(v, 123); // arg1 is a lvalue reference, arg2 is a rvalue reference.
CallTwo(123, v); // arg1 is a rvalue reference, arg2 is a lvalue reference.
```
...
```
There are two overloads for `std::forward`. One for forwarding an lvalue and another for forwarding an rvalue. How the compiler distinguish between these two overloads? In fact the complier can not deduce the type of template parameter `_Tp`. This why we should call `std::forward<T1>(arg1)` instead of `std::forward(arg1)`. The key is that we explicitly specifying the template parameter. With properly specifying the template parameter, it can be ensured that the first overload accepts lvalue (recall that both lvalue reference and rvalue reference are lvalues.) and the second overload accepts rvalue.

 Do you still remember when a universal reference is exactly a lvalue reference, the template parameter is actually a lvalue reference type? Here, if `arg1` has type of `int&` then `T1` is `int&`. Otherwise if `arg1` has type of `int&&` then `T1` is `int`. `typename std::remove_reference<_Tp>::type&` will always result a lvalue reference, meanwhile `_Tp` can either be a lvalue reference type or a non reference type. When the passed parameter is a lvalue reference, `_Tp` is a lvalue reference type which results return type `_Tp&&` is lvalue reference. Alternatively, when the passed parameter is a rvalue reference, `_Tp` is a non reference type which results return type `_Tp&&` is rvalue reference.

So the invokes in `CallTwo` always match the first overload. Sometimes we need to pass a rvalue to `std::forward`, e.g., using `std::forward` in a macro and the macro arguments is `std::move(...)` or literal. In this case, the matched overload will be the second one. Since the second overload always accepts rvalue, its return type should always be rvalue reference, too. So we have a static_assert here to ensure the `_Tp` is not a lvalue reference type. The invoking such as `std::forward<int&>(123)` should be fail to compile.